...
const PrivateMessagesPanel: React.FC<PrivateMessagesPanelProps> = ({
  ...
  useEffect(() => {
    const checkMobile = () => {
      ...
      setIsMobile(mobile);
      // On mobile, show list by default if no recipient selected
      if (mobile && !selectedRecipient) {
        setShowChatView(false);
      }
      // On desktop, always show chat if recipient is selected
      if (!mobile && selectedRecipient) {
  ...
  const handleBackToList = () => {
    if (isMobile) {
      setShowChatView(false);
      // Keep selectedRecipient so user can see who they're chatting with when they go back
    }
  };
  ...
  const conversationMap = useMemo(() => {
    ...
    conversations.forEach(conv => {
      map.set(conv.participantId, conv);
    });
    return map;
  }, [conversations]);
  ...
  return (
    <div className="flex h-full min-h-0">
      {/* Participants/Conversations sidebar - WhatsApp style - Always visible on desktop, hidden on mobile when chat is open */}
      <div className={`${isMobile && showChatView ? 'hidden' : 'flex'} ${isMobile ? 'w-full' : 'w-64 sm:w-80'} border-r border-gray-300 bg-white flex-col flex-shrink-0`}>
        ...
        {/* Participants/Conversations list - Show all participants, highlight those with conversations */}
        <div className="flex-1 overflow-y-auto">
          {availableParticipants.length === 0 ? (
            ...
            availableParticipants.map((participant) => {
              const conv = conversationMap.get(participant.uid);
              const isSelected = selectedRecipient === participant.uid;
              const hasConversation = !!conv;
              const lastMessagePreview = conv ? getLastMessagePreview(conv.lastMessage) : '';
              
              return (
                <button
                  key={participant.uid}
                  onClick={() => handleSelectRecipient(participant.uid)}
      ...
      {/* Chat area - Hidden on mobile when list is shown, always visible on desktop if recipient selected */}
      <div className={`${isMobile && !showChatView ? 'hidden' : 'flex'} ${!selectedRecipient ? 'hidden' : ''} flex-1 flex-col min-w-0`}>
        ...
        {selectedRecipient ? (
          <>
            <div className="p-3 bg-white border-b border-gray-200 flex items-center justify-between flex-shrink-0">
              ...
              {/* Desktop: Clear selection button */}
              {!isMobile && (
                <button
            ...
            <div className="border-t border-gray-700 bg-gray-800 flex-shrink-0">
                    ...
                    {uploadingFiles.map((file, index) => (
                      <div key={index}>
                        <div className="flex items-center justify-between mb-1">
                          ...
                          <span className="text-xs text-gray-300">{Math.round(file.progress)}%</span>
                        </div>
              ...
              {replyingTo && (
                <div className="px-3 pt-2 pb-1">
                  <div className="p-2 bg-gray-700 rounded-lg border-l-2 border-techBlue flex items-start justify-between">
                      ...
                      <div className="flex-shrink-0">
                        {replyingTo.files && replyingTo.files.length > 0 ? (
                          replyingTo.files[0].type === 'image' ? (
                            ...
                            <div className="w-12 h-12 rounded bg-gray-600 relative overflow-hidden">
                              <video 
                                src={replyingTo.files[0].url} 
                                className="w-full h-full object-cover"
                              />
              ...
              <form onSubmit={handleSubmit} className="px-2 py-2 relative">
                {/* Emoji Picker */}
                {showEmojiPicker && (
                  <div 
                    ref={emojiPickerRef}
                ...
                <div className="flex items-center space-x-1">
                  ...
                  <button
                    ...
                    className="text-gray-400 hover:text-white p-1.5 disabled:opacity-50 transition-colors flex-shrink-0"
                    title="Emoji"
                  >
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <textarea
                    ...
                    className="flex-1 bg-gray-800 px-4 py-2.5 text-white placeholder-gray-400 resize-none focus:outline-none text-sm leading-5 min-h-[36px] max-h-[120px] overflow-y-auto"
                    style={{ 
                      minHeight: '36px',
                      maxHeight: '120px',
                      height: '36px',
                  ...
                  <button
                    type="submit"
                    disabled={(!newMessage.trim() && !uploading) || uploading || newMessage.length > MAX_LENGTH}
                    className="bg-techBlue text-cloud rounded-full p-2 disabled:opacity-50 disabled:cursor-not-allowed hover:bg-techBlue/90 transition-colors flex-shrink-0"
